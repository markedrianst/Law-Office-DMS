<template>
  <div class="min-h-screen p-4 sm:p-6 bg-slate-50">
    ...
    <div v-if="pendingApprovals.length > 0 && userRole === 'lawyer'" class="mb-4">
      <div class="bg-amber-50 border-l-4 border-amber-400 rounded-r-2xl p-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <button @click="showPendingModal = true"
          class="shrink-0 px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-semibold rounded-xl transition active:scale-95">
          Review Now
        </button>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-3 mb-4 space-y-2">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <select v-model="filters.approval_status" @change="handleFilterChange"
          class="flex-1 min-w-[110px] px-3 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-[#1a4972] text-slate-600 cursor-pointer">
        ...
        <select v-model="filters.is_active" @change="handleFilterChange"
          class="flex-1 min-w-[100px] px-3 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-[#1a4972] text-slate-600 cursor-pointer">
    ...
    <div v-else class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div class="block sm:hidden">
        <div v-if="documents.length === 0 && !isRefreshing" class="flex flex-col items-center py-14 px-6">
          ...
          <div v-for="item in documents" :key="item.id"
            ...
            <div class="flex items-start justify-between gap-3 mb-2">
              <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                  <p class="text-sm font-bold text-slate-800 truncate">{{ item.type }}</p>
                  <span class="inline-block mt-0.5 px-2 py-0.5 text-[10px] font-bold rounded-full" :class="categoryBadgeClass(item.category)">{{ item.category }}</span>
                </div>
              </div>
            ...
            <div class="flex flex-wrap items-center gap-2 mb-3">
              <div v-if="item.requires_approval">
                <span class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-lg"
              ...
              <span class="text-[11px] text-slate-400">Sort: {{ item.sort_order }} · {{ formatDateHelper(item.created_at) }}</span>
            </div>

            <!-- Actions -->
            <div class="space-y-2">
              <!-- Approve/Reject (lawyers only, pending docs) -->
              <div v-if="item.requires_approval && item.approval_status === 'pending' && userRole === 'lawyer'" class="flex gap-2">
                <button @click="approveDocument(item)" :disabled="approvingId === item.id"
                  class="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-100 transition-all active:scale-95 disabled:opacity-50">
                  <svg v-if="approvingId !== item.id" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button @click="showRejectModal(item)" :disabled="rejectingId === item.id"
                  ...
                  <svg v-if="rejectingId !== item.id" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <table class="min-w-full">
            ...
            <tr class="border-b border-slate-100 bg-[#1a4972]/5">
              <th v-for="col in columns" :key="col.field" scope="col"
                ...
                <div class="flex items-center gap-1.5">
                  {{ col.label }}
                  <svg v-if="col.sortable && filters.sort_by === col.field" class="w-3.5 h-3.5 text-[#1a4972]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <tbody class="divide-y divide-slate-50">
            <tr v-for="(item, index) in documents" :key="item.id"
              ...
              <td class="px-5 py-4">
                <span class="inline-block px-2.5 py-1 text-xs font-semibold rounded-full" :class="categoryBadgeClass(item.category)">{{ item.category }}</span>
              </td>
              <td class="px-5 py-4">
                <div v-if="item.requires_approval" class="flex flex-col gap-1">
                  <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-lg" :class="approvalStatusClass(item.approval_status)">
                    <span class="w-1.5 h-1.5 rounded-full" :class="approvalStatusDot(item.approval_status)"></span>
                    {{ formatApprovalStatus(item.approval_status) }}
                  </span>
                  <span v-if="item.approval_status === 'approved' && item.approver" class="text-[10px] text-slate-400">by {{ item.approver.full_name }}</span>
                  <span v-if="item.approval_status === 'rejected' && item.rejection_reason" class="text-[10px] text-red-400 truncate max-w-[150px]" :title="item.rejection_reason">{{ item.rejection_reason }}</span>
                </div>
                <span v-else class="text-sm text-slate-400">—</span>
              </td>
              <td class="px-5 py-4 text-sm text-slate-600">{{ item.sort_order }}</td>
              <td class="px-5 py-4">
                <span class="px-2.5 py-1 text-xs font-semibold rounded-lg" :class="item.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'">{{ item.is_active ? 'Active' : 'Inactive' }}</span>
              </td>
        ...
        <div class="relative bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[90dvh] flex flex-col">
          <div class="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-slate-100 flex-shrink-0">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h2 class="text-base font-bold text-slate-800">{{ isEditing ? 'Edit Document' : 'Add Document' }}</h2>
                <p class="text-xs text-slate-500">{{ isEditing ? 'Update document type' : 'Create a new document type' }}</p>
              </div>
            </div>
          </div>
          <div class="px-5 sm:px-6 py-5 space-y-4 overflow-y-auto">
            ...
            <div class="flex items-center gap-2">
              <input type="checkbox" v-model="form.requires_approval" id="requiresApproval" class="w-4 h-4 rounded border-slate-300 text-[#1a4972] focus:ring-[#1a4972]"/>
              <label for="requiresApproval" class="text-sm font-medium text-slate-700">Requires Lawyer Approval</label>
            </div>
            <div>
    ...
    <Transition name="modal">
      <div v-if="showPendingModal" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4" @click.self="showPendingModal = false">
        <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm"></div>
        <div class="relative bg-white w-full sm:max-w-2xl rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[90dvh] flex flex-col">
          <div class="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-slate-100 flex-shrink-0">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h2 class="text-base font-bold text-slate-800">Pending Approvals</h2>
                <p class="text-xs text-slate-500">Review and approve/reject document types</p>
              </div>
            </div>
            <button @click="showPendingModal = false" class="w-8 h-8 rounded-xl flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all">
          ...
          <div class="px-5 sm:px-6 py-5 overflow-y-auto">
            ...
            <div v-else class="space-y-3">
              <div v-for="doc in pendingApprovals" :key="doc.id"
                class="flex flex-wrap items-center justify-between gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                <div class="flex items-center gap-3 min-w-0">
                  <div class="w-8 h-8 rounded-full shrink-0" :style="{ backgroundColor: doc.color }"></div>
                  <div class="min-w-0">
                ...
                <div class="flex items-center gap-2 shrink-0">
                  <button @click="approveDocument(doc)" :disabled="approvingId === doc.id"
                    class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl transition flex items-center gap-1 active:scale-95">
                  ...
                  <button @click="showRejectModal(doc)" :disabled="rejectingId === doc.id"
                    class="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-xl transition flex items-center gap-1 active:scale-95">
    ...
    <!-- Reject Modal -->
    <Transition name="modal">
      <div v-if="showRejectDocModal" class="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:p-4" @click.self="showRejectDocModal = false">
        <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm"></div>
        <div class="relative bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden">
          <div class="px-5 sm:px-6 py-4 border-b border-slate-100">
            <h3 class="text-base font-bold text-slate-800">Reject Document</h3>
          ...
          <div class="flex items-center justify-end gap-3 px-5 sm:px-6 py-4 border-t border-slate-100 bg-slate-50/50">
            ...
            <button @click="submitRejection" :disabled="rejectLoading || !rejectionReason.trim()"
              class="px-5 py-2.5 text-sm font-semibold text-white bg-red-600 rounded-xl hover:bg-red-700 transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50">
...
const lastUpdated = ref(initialDocuments?.length ? new Date().toLocaleTimeString() : 'No data');
...
const pagination = ref({
  ...
  from: 1, to: Math.min(15, initialDocuments?.length || 0)
});

const filters = reactive({
  search: '', category: '', approval_status: '', is_active: '',
  sort_by: 'sort_order', sort_direction: 'asc'
});
...
const form = reactive({
  type: '', category: '', color: '#94a3b8', requires_approval: true, sort_order: null, is_active: true
});
const errors = reactive({ type: '', category: '', color: '', sort_order: '' });
...
const displayedPages = computed(() => {
  ...
  if (total <= max) { for (let i = 1; i <= total; i++) pages.push(i); }
    ...
    let start = Math.max(1, current - 2); let end = Math.min(total, start + max - 1);
...
const approvalStatusClass = (status) => {
  const classes = { 'pending': 'bg-amber-50 text-amber-700 border border-amber-200', 'approved': 'bg-emerald-50 text-emerald-700 border border-emerald-200', 'rejected': 'bg-red-50 text-red-700 border border-red-200' };
  return classes[status] || 'bg-slate-50 text-slate-600';
};

const approvalStatusDot = (status) => {
  const classes = { 'pending': 'bg-amber-500', 'approved': 'bg-emerald-500', 'rejected': 'bg-red-500' };
...
const fetchDocuments = async () => {
  ...
  try {
    const params = { search: filters.search || undefined, category: filters.category || undefined, approval_status: filters.approval_status || undefined, is_active: filters.is_active || undefined, sort_by: filters.sort_by, sort_direction: filters.sort_direction, page: pagination.value.current_page, per_page: pagination.value.per_page };
    const response = await documentService.getDocuments(params);
    if (response.data) {
      documents.value = response.data; pagination.value = response.meta;
...
const fetchPendingApprovals = async () => {
  ...
  try { const response = await documentService.getPendingApprovals(); pendingApprovals.value = response.data || []; }
  catch (error) { console.error('Failed to load pending approvals:', error); }
};

const fetchDocumentCategories = async () => {
  try { const response = await documentService.getDocumentCategories(); documentCategories.value = response.data || ['Pleading', 'Letter', 'Evidence', 'Court Issuance', 'Other']; }
  catch (error) { console.error('Failed to load document categories:', error); }
};

const initialize = async () => {
  if (!initialDocuments?.length) { isLoading.value = true; await fetchDocumentCategories(); }
  else { documents.value = initialDocuments.slice(0, pagination.value.per_page); pagination.value.total = initialDocuments.length; pagination.value.last_page = Math.ceil(initialDocuments.length / pagination.value.per_page); }
  await fetchDocuments(); isLoading.value = false;
  if (userRole.value === 'lawyer') await fetchPendingApprovals();
};

const debouncedSearch = debounce(() => { pagination.value.current_page = 1; fetchDocuments(); }, 500);
const handleFilterChange = () => { pagination.value.current_page = 1; fetchDocuments(); };
const sortBy = (field) => {
  if (filters.sort_by === field) { filters.sort_direction = filters.sort_direction === 'asc' ? 'desc' : 'asc'; }
  else { filters.sort_by = field; filters.sort_direction = 'asc'; }
  pagination.value.current_page = 1; fetchDocuments();
};
const previousPage = () => { if (pagination.value.current_page > 1) { pagination.value.current_page--; fetchDocuments(); } };
const nextPage = () => { if (pagination.value.current_page < pagination.value.last_page) { pagination.value.current_page++; fetchDocuments(); } };
const goToPage = (page) => { pagination.value.current_page = page; fetchDocuments(); };

const resetForm = () => {
  form.type = ''; form.category = ''; form.color = '#94a3b8'; form.requires_approval = true; form.sort_order = null; form.is_active = true;
  errors.type = ''; errors.category = ''; errors.color = ''; errors.sort_order = '';
};
const clearErrors = () => { errors.type = ''; errors.category = ''; errors.color = ''; errors.sort_order = ''; };

const openCreateModal = () => { resetForm(); isEditing.value = false; editingItemId.value = null; showModal.value = true; };
const editItem = (item) => {
  resetForm(); isEditing.value = true; editingItemId.value = item.id;
  form.type = item.type; form.category = item.category; form.color = item.color;
  form.requires_approval = item.requires_approval; form.sort_order = item.sort_order; form.is_active = item.is_active;
...
const submitForm = async () => {
  ...
  try {
    const payload = { type: form.type, category: form.category, color: form.color, requires_approval: form.requires_approval, sort_order: form.sort_order, is_active: form.is_active };
    if (isEditing.value) {
      editingId.value = editingItemId.value;
      await documentService.updateDocument(editingItemId.value, payload);
      Swal.fire({ icon: 'success', title: 'Success!', text: 'Document updated successfully', timer: 1500, showConfirmButton: false, position: 'top-end', toast: true });
    } else {
      isAdding.value = true;
      await documentService.createDocument(payload);
      Swal.fire({ icon: 'success', title: 'Success!', text: 'Document created successfully', timer: 2000, showConfirmButton: false, position: 'top-end', toast: true });
    }
    closeModal(); await fetchDocuments();
    if (userRole.value === 'lawyer') await fetchPendingApprovals();
  } catch (error) {
    if (error.errors) {
  ...
  } finally { formLoading.value = false; isAdding.value = false; editingId.value = null; }
};
...
const approveDocument = async (item) => {
  ...
  try {
    ...
    Swal.fire({ icon: 'success', title: 'Approved!', text: 'Document approved successfully', timer: 1500, showConfirmButton: false, position: 'top-end', toast: true });
  } catch (error) {
    Swal.fire({ icon: 'error', title: 'Error!', text: error.message || 'Failed to approve document', timer: 2000, showConfirmButton: false, position: 'top-end', toast: true });
  } finally { approvingId.value = null; }
};

const showRejectModal = (item) => { documentToReject.value = item; rejectionReason.value = ''; showRejectDocModal.value = true; };
const submitRejection = async () => {
  if (!rejectionReason.value.trim() || !documentToReject.value) return;
  rejectLoading.value = true; rejectingId.value = documentToReject.value.id;
  try {
    await documentService.rejectDocument(documentToReject.value.id, { rejection_reason: rejectionReason.value });
    pendingApprovals.value = pendingApprovals.value.filter(p => p.id !== documentToReject.value.id);
    await fetchDocuments(); showRejectDocModal.value = false;
    Swal.fire({ icon: 'success', title: 'Rejected', text: 'Document rejected', timer: 1500, showConfirmButton: false, position: 'top-end', toast: true });
  } catch (error) {
    Swal.fire({ icon: 'error', title: 'Error!', text: error.message || 'Failed to reject document', timer: 2000, showConfirmButton: false, position: 'top-end', toast: true });
  } finally { rejectLoading.value = false; rejectingId.value = null; documentToReject.value = null; }
};

const toggleStatus = async (item) => {
  togglingId.value = item.id;
  try {
    await documentService.toggleDocument(item.id); await fetchDocuments();
    Swal.fire({ icon: 'success', title: 'Success!', text: `Document ${!item.is_active ? 'activated' : 'deactivated'} successfully`, timer: 1500, showConfirmButton: false, position: 'top-end', toast: true });
  } catch (error) {
...
const confirmDelete = async (item) => {
  const result = await Swal.fire({ title: 'Delete Document?', text: `Are you sure you want to delete "${item.type}"?`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#dc2626', cancelButtonColor: '#64748b', confirmButtonText: 'Yes, delete', cancelButtonText: 'Cancel' });
  if (result.isConfirmed) {
    ...
    try {
      ...
      pendingApprovals.value = pendingApprovals.value.filter(p => p.id !== item.id);
      await fetchDocuments();
      Swal.fire({ icon: 'success', title: 'Deleted!', text: 'Document deleted successfully', timer: 1500, showConfirmButton: false, position: 'top-end', toast: true });
    } catch (error) {
      Swal.fire({ icon: 'error', title: 'Error!', text: error.message || 'Failed to delete document', timer: 2000, showConfirmButton: false, position: 'top-end', toast: true });
    } finally { deletingId.value = null; }
  }
};

watch(() => filters.search, () => { debouncedSearch(); });
watch(() => filters.category, () => { handleFilterChange(); });
watch(() => filters.approval_status, () => { handleFilterChange(); });
watch(() => filters.is_active, () => { handleFilterChange(); });

const handleDocumentsUpdated = (event) => {
  allDocuments.value = event.detail;
  if (pagination.value.current_page === 1) documents.value = event.detail.slice(0, pagination.value.per_page);
};
